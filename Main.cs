...
namespace GorillaFriends
{
    ...
    public class Main : BaseUnityPlugin
    {
        ...
        // Config
        public static Color m_clrFriend { get; internal set; } = new Color(1.0f, 0.0f, 0.0f);
                    ...
                    if (__instance.lines[i].gameObject.activeInHierarchy)
                    {
                        ...
                        if (__instance.lines[i].linePlayer != null)
                        {
                            ...
                            if (__instance.lines[i].linePlayer != PhotonNetwork.LocalPlayer)
                            {
                                ...
                                {
                                    __instance.buttonText.text += "BLOCK       MUTE                      REPORT\n";
                                }
                                else
                                {
                                    __instance.buttonText.text += "BLOCK       MUTE      HATE SPEECH    TOXICITY      CHEATING      CANCEL\n";
                                }
                            }
                            else
                            {
